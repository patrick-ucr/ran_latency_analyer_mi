...
class LteMacAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.totalDlTbsBytes = 0
        self.totalUlGntBytes = 0
                ...
                for i in range(0, len(log_item['Subpackets'])):
                    if log_item['Subpackets'][i].has_key('Samples'):
                        ...
                        for sample in log_item['Subpackets'][i]['Samples']:
                            if(sample.has_key('DL TBS (bytes)')):
                                self.totalDlTbsBytes += int(sample['DL TBS (bytes)'])
            self.log_info(timestamp + " Total DL TBS (bytes): " + str(self.totalDlTbsBytes))
            ...
            if log_item.has_key('Subpackets'):
                for i in range(0, len(log_item['Subpackets'])):
                    if log_item['Subpackets'][i].has_key('Samples'):
                        # print log_item
                        for sample in log_item['Subpackets'][i]['Samples']:
                            if(sample.has_key('Grant (bytes)')):
                                self.totalUlGntBytes += int(sample['Grant (bytes)'])
                                self.log_info("MAC_UL_TB: "+timestamp+ " SubId: " +str(sample['Sub Id'])+" CellId: "+str(sample['Cell Id'])+" SysFN: "+str(sample['SFN'])+" subFN: "+str(sample['Sub-FN'])+" Grant(bytes): "+str(sample['Grant (bytes)'])+" RLCPDUs: "+str(sample['RLC PDUs'])+" Padding(bytes): "+str(sample['Padding (bytes)'])+" BSR_event: "+str(sample['BSR event'])+" BSR_trig: "+str(sample['BSR trig'])+" HDR_LEN: "+str(sample['HDR LEN']))
                            else:
                                self.log_info("MAC_UL_TB: "+timestamp+ " SubId: " +str(sample['Sub Id'])+" CellId: "+str(sample['Cell Id'])+" SysFN: "+str(sample['SFN'])+" subFN: "+str(sample['Sub-FN'])+" RLCPDUs: "+str(sample['RLC PDUs'])+" Padding(bytes): "+str(sample['Padding (bytes)'])+" BSR_event: "+str(sample['BSR event'])+" BSR_trig: "+str(sample['BSR trig'])+" HDR_LEN: "+str(sample['HDR LEN']))

            self.log_info("[MAC UL TB Info]: "+timestamp + " Total UL Grant (bytes): " + str(self.totalUlGntBytes))
