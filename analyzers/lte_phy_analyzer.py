...
class LtePhyAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        # Resource slot used by SR (Scheduling Request)
        self.rb_slot1 = None
        self.rb_slot2 = None
    ...
    def callback_pusch_tx(self, msg):
        ...
        #     print record
            ...
            numRB = record['Num of RB']
            tbSize = record['PUSCH TB Size']
            totalRB += int(numRB)
            totalTBSize += int(tbSize)
            coderate = record['Coding Rate']
            curSFNSF = str(record['Current SFN SF'])
            bcast_dict = {}
            bcast_dict['tx power'] = pusch_tx_power
            bcast_dict['timestamp'] = timestamp
            self.broadcast_info("PUSCH_TX_POWER", bcast_dict)
            self.log_info("[PUSCH Tx Info] "+timestamp+" Current SFN SF: "+curSFNSF+" PUSCH_TX_POWER: " + str(pusch_tx_power)+" Num RBs: "+str(numRB)+" TB Size: "+str(tbSize) + " Mod: " + record['PUSCH Mod Order'] + " Coding Rate: %.3f"%coderate)
        self.log_info("[PUSCH Total Tx Info]: "+timestamp+" Total RBs: " + str(totalRB)+" Total TB Sizes: "+str(totalTBSize))
