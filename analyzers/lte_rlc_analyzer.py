...
class LteRlcAnalyzer(Analyzer):
    ...
    def __init__(self):
        ...
        self.dlPduData = 0
        self.dlPduCtrl = 0
        self.ulPduDataSn = set() # set of uplink pdu sequence numbers
        self.dlPduDataSn = set()
    ...
    #def callback_rlcstats(self,msg):
    #    log_item = msg.data.decode()
    #    self.log_info("LOG: RLC Stats")
    #    self.broadcast_info("BRC: RLC Stats")
    ...
    def __msg_callback(self, msg):
        ...
        if msg.type_id == "LTE_RLC_UL_AM_All_PDU":
            ...
            self.ulPduData = 0
            self.ulPduCtrl = 0
            self.ulPduDataSn = set()
            self.log_info("[RLC UL PDU Start]")
            for pduItem in listPDU:
                if pduItem['PDU TYPE'] == 'RLCUL DATA':
                    self.rbInfo[rbConfigIdx]['cumulativeULData'] += \
                        int(pduItem['pdu_bytes'])
                    self.ulPduData += int(pduItem['pdu_bytes'])
                    SN = int(pduItem['SN'])
                    self.ulPduDataSn.add(str(SN))
                    sys_fn = int(pduItem['sys_fn'])
                    sub_fn = int(pduItem['sub_fn'])
                    fi = str(pduItem['FI'])
                    e = str(pduItem['E'])
                    pdu_bytes = str(pduItem['pdu_bytes'])
                    self.log_info("[RLC UL PDU Info]\t"+str(log_item['timestamp'])+" SN: "+str(SN)+" FI: "+fi+" E: "+e+" sys_fn: "+str(sys_fn)+" sub_fn: "+str(sub_fn)+" pdu_bytes: "+pdu_bytes+ " rb_cfg_idx: "+str(rbConfigIdx))
                    ...
                    self.rbInfo[rbConfigIdx]['cumulativeULData'] += \
                        int(pduItem['pdu_bytes'])
                    self.ulPduCtrl += int(pduItem['pdu_bytes'])
                    AckSN = pduItem['SN']
                    AckSN = int(AckSN.split(" = ")[1])
                    sys_fn = int(pduItem['sys_fn'])
                    sub_fn = int(pduItem['sub_fn'])
                    rbConfigIdx = int(pduItem['rb_cfg_idx'])
                    pdu_bytes = int(pduItem['pdu_bytes'])
                    self.log_info("[RLC UL PDU Ctrl]\t"+str(log_item['timestamp'])+" sys_fn: "+str(sys_fn)+" sub_fn: "+str(sub_fn)+" pdu_bytes: "+str(pdu_bytes)+ " rb_cfg_idx: "+str(rbConfigIdx))
                    if sys_fn > maxSys_fn or (
                            sys_fn == maxSys_fn and sub_fn > maxSub_fn):
                        maxSys_fn = sys_fn
                        maxSub_fn = sub_fn
            ...
            self.log_info("[RLC UL PDU End]")
            self.log_info("[RLC UL Total PDU (bytes)]: "+str(log_item['timestamp']) + " Data: "+str(self.ulPduData)+" Ctrl: " + str(self.ulPduCtrl))
            #self.log_info("[UL PDU SN List]\t"+"Timestamp: "+str(log_item['timestamp']) + " SN List: "+repr(self.ulPduDataSn))
            ...
            for pduItem in listPDU:
                if pduItem['PDU TYPE'] == 'RLCDL DATA':
                    self.rbInfo[rbConfigIdx]['cumulativeDLData'] += \
                        int(pduItem['pdu_bytes'])
                    self.dlPduData += int(pduItem['pdu_bytes'])
                    SN = int(pduItem['SN'])
                    self.dlPduDataSn.add(str(SN))
                    sys_fn = int(pduItem['sys_fn'])
                    sub_fn = int(pduItem['sub_fn'])
                    if sys_fn > maxSys_fn or (
                            sys_fn == maxSys_fn and sub_fn > maxSub_fn):
                    ...
                    self.rbInfo[rbConfigIdx]['cumulativeDLData'] += int(
                        pduItem['pdu_bytes'])
                    self.dlPduCtrl += int(pduItem['pdu_bytes'])
                    AckSN = pduItem['SN']
                    AckSN = int(AckSN.split(" = ")[1])
                    sys_fn = int(pduItem['sys_fn'])
                    sub_fn = int(pduItem['sub_fn'])
                        ...
                        self.rbInfo[rbConfigIdx]['UL']['listAck'].append(
                            {
                                ...
                                'sub_fn': pduItem['sub_fn'],
                                'time_stamp': log_item['timestamp']})
            self.log_info("[RLC DL Total PDU (bytes)]\t"+"Timestamp: "+str(log_item['timestamp']) + " Data: "+str(self.dlPduData)+" Ctrl: " + str(self.dlPduCtrl))
            #self.log_info("[DL PDU SN List]\t"+"Timestamp: "+str(log_item['timestamp']) + " SN List: "+repr(self.dlPduDataSn))
